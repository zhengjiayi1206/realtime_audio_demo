...
from wenet.utils.checkpoint import load_checkpoint, load_encoder_checkpoint, load_trained_modules, load_link_llm_checkpoint
...
def init_llmasr(args, configs, is_inference=False):
    ...
    if hasattr(args, 'checkpoint') and args.checkpoint is not None:
        ...
        #infos = load_encoder_checkpoint(model, args.checkpoint)
    ...
    if not is_inference:
        ...
        # logging.info('开始更换LLM的参数')
        # checkpoint4llm_wrapper = "./step_34272.pt"
        # load_checkpoint(model, checkpoint4llm_wrapper)
        # logging.info('更换LLM的参数完毕')
    else:
        logging.info('不更换LLM的参数')
    ...
    for k, p in model.named_parameters():
        ...
        if fire_module == 'link':
            ...
            if not k.startswith("llama_model"):
                p.requires_grad = False
            #if k.startswith("speech_token_emded") or k.startswith("speaker_head"):
            #    p.requires_grad = True
        elif fire_module == 'link_and_encoder':
            # 这里和speech token相关的层不会被冻结
            if k.startswith("llama_model"):
                p.requires_grad = False
        elif fire_module == 'link_and_llm':
            if k.startswith("speech_token_emded") or k.startswith("speaker_head") or k.startswith("encoder"):
                p.requires_grad = False # speech_llama_proj
        elif fire_module == "encoder_and_link_and_llm":
            logging.info('所有参数均解冻')
            break
