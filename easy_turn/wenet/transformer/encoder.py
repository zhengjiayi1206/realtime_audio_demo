...
class BaseEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        norm_eps: float = 1e-5,
        output_hidden_states: bool = False,  # 新增标志位
    ):
        """
        Args:
        ...
        self.output_hidden_states = output_hidden_states  # 新增
    ...
    def forward(
    ...
    ) -> Tuple[torch.Tensor, torch.Tensor, Optional[Tuple[torch.Tensor]]]:  # 修改返回类型
        ...
        if self.gradient_checkpointing and self.training:
            xs, hidden_states = self.forward_layers_checkpointed(xs, chunk_masks, pos_emb,
                                                                 mask_pad, output_hidden_states)
            #xs = self.forward_layers_checkpointed(xs, chunk_masks, pos_emb,
            #                                      mask_pad)
        else:
            xs, hidden_states = self.forward_layers(xs, chunk_masks, pos_emb, mask_pad,
                                                   output_hidden_states)
            #xs = self.forward_layers(xs, chunk_masks, pos_emb, mask_pad)
        if self.normalize_before:
            xs = self.after_norm(xs)
            if output_hidden_states:
                hidden_states = [self.after_norm(h) for h in hidden_states]
        # Here we assume the mask is not changed in encoder layers, so just
        # return the masks before encoder layers, and the masks will be used
        # for cross attention with decoder later
        return (xs, masks, hidden_states) if output_hidden_states else (xs, masks)
        #return xs, masks
    
    def forward_layers(self, xs: torch.Tensor, chunk_masks: torch.Tensor,
                      pos_emb: torch.Tensor, mask_pad: torch.Tensor,
                      output_hidden_states: bool) -> Tuple[torch.Tensor, list[torch.Tensor]]:
        hidden_states = []
        for layer in self.encoders:
            xs, chunk_masks, _, _ = layer(xs, chunk_masks, pos_emb, mask_pad)
            if output_hidden_states:
                hidden_states.append(xs.clone())
        return xs, hidden_states

    @torch.jit.unused
    def forward_layers_checkpointed(self, xs: torch.Tensor, chunk_masks: torch.Tensor,
                                   pos_emb: torch.Tensor, mask_pad: torch.Tensor,
                                   output_hidden_states: bool) -> Tuple[torch.Tensor, list[torch.Tensor]]:
        hidden_states = []
        for layer in self.encoders:
...
class TransformerEncoder(BaseEncoder):
    ...
    def __init__(
        ...
        super().__init__(input_size, output_size, attention_heads,
                         ...
                         use_sdpa, layer_norm_type, norm_eps, output_hidden_states=output_hidden_states)
