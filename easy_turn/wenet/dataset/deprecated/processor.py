...
def extract_style_from_text(text): #不带<>
    """从文本中提取风格标签"""
    # 使用正则匹配尖括号中的内容
    match = re.search(r'<(.*?)>$', text)
    return match.group(1) if match else "COMPLETE"
                    ...
                    try:
                        if postfix == 'txt':
                            ...
                            example['task'] = file_obj.read().decode(
                                'utf8').strip()
                            if example['task'] == '<TRANSCRIBE> <BACKCHANNEL>' or example['task'] == '<TRANSCRIBE> <COMPLETE>':
                                example['task'] = "<TRANSCRIBE> <BACKCHANNEL> <COMPLETE>"
                                #print(example['task'])
...
def convert_brackets_to_lowercase(txt):
    def to_lowercase(match):
        return match.group(0).lower()    
    result_txt = re.sub(r'<[^>]+>', to_lowercase, txt)
    return result_txt

"""
def remove_punctuation_except_tags(text):

    #移除文本中除了标签（如<COMPLETE>）之外的所有标点符号和下划线
    ...
    for char in text:
        if char == '<':
            ...
            if current_segment:
                ...
                result.append(punct_pattern.sub('', segment))
                current_segment = []
            in_tag = True
            result.append(char)
        elif char == '>':
...
def remove_punctuation_except_tags(text):
    # 匹配尖括号内的标签（如<COMPLETE>）
    tag_pattern = re.compile(r'<.*?>')
    # 匹配中英文标点符号（不包含尖括号）
    #punct_pattern = re.compile(r'[^\w\s<>]|_')  # 移除下划线和所有非字母、数字、空格、<>
    punct_pattern = re.compile(
        r'[\u3000-\u303F\uFF00-\uFFEF\u2000-\u206F'  # 中文标点范围
        r'!\"#$%&\'()*+,.-/:;<=>?@[\\\]^`{|}~]'     # 英文标点范围
        r'|_'                                       # 单独匹配下划线
    ...
    # 处理普通文本部分：去除非字母、数字、空格和<>的符号
    cleaned_parts = [punct_pattern.sub('', part) for part in parts]
    
    # 重新将标签插入到对应位置
    result = []
    for i in range(len(cleaned_parts)):
        result.append(cleaned_parts[i])
        if i < len(tags):
...
def tokenize(data, tokenizer: BaseTokenizer, global_prompt_dict=None):
    ...
    for sample in data:
        ...
        if 'task' in sample:
            ...
            elif "<BACKCHANNEL>" in sample['task'] or "<COMPLETE>" in sample['task']:
                txt = convert_brackets_to_lowercase(sample['txt'])
                txt = remove_punctuation_except_tags(txt)
            else:
                txt = sample['txt']
        else:
            txt = sample['txt']
...
def padding(data):
    ...
    for sample in data:
        ...
        sorted_style = [
            sample[i].get('style', 'None') for i in order
        ]
        sorted_style_tensor = torch.tensor([x for x in sorted_style],
                                   dtype=torch.int64)
