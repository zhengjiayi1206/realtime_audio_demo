...
def load_encoder_checkpoint(model: torch.nn.Module, path: str) -> dict:
    rank = int(os.environ.get('RANK', 0))
    ...
    # 3. 获取 checkpoint 中的 state_dict
    checkpoint_state_dict = checkpoint
    
    # 4. 过滤出以 "encoder" 开头的参数
    filtered_state_dict = {k: v for k, v in checkpoint_state_dict.items() if k.startswith('encoder')}
    # ln_speech speech_transformer down_sample_2
    # 5. 更新模型的状态字典
...
def load_link_llm_checkpoint(model: torch.nn.Module, path: str) -> dict:
    rank = int(os.environ.get('RANK', 0))
    logging.info('[Rank {}] Checkpoint: loading from checkpoint {}'.format(rank, path))
    ...
    # 4. 过滤出以 "encoder" 开头的参数
    filtered_state_dict = {k: v for k, v in checkpoint_state_dict.items() if not k.startswith('encoder')}
    # ln_speech speech_transformer down_sample_2
    # 5. 更新模型的状态字典
    missing_keys, unexpected_keys = model.load_state_dict(filtered_state_dict, strict=False)
