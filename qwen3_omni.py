...
class Qwen3OmniMoeForConditionalGeneration(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.debug_realtime_tokens = _env_flag("QWEN3_OMNI_DEBUG_REALTIME_TOKENS", default=False)
        self.debug_realtime_decode = _env_flag("QWEN3_OMNI_DEBUG_REALTIME_DECODE", default=True)
        self.debug_realtime_token_limit = max(1, int(os.getenv("QWEN3_OMNI_DEBUG_REALTIME_TOKEN_LIMIT", "128")))
    ...
    def debug_tokenizer(self):
        try:
            return cached_tokenizer_from_config(self.vllm_config.model_config)
        except Exception:
            logger.warning("Failed to load tokenizer for realtime token debug.", exc_info=True)
            return None

    def _is_realtime_debug_enabled(self) -> bool:
        return bool(self.debug_realtime_tokens)

    def _flatten_token_ids(self, value: Any) -> list[int]:
        if value is None:
            return []
        if isinstance(value, torch.Tensor):
            return [int(item) for item in value.detach().reshape(-1).tolist()]
        if isinstance(value, np.ndarray):
            return [int(item) for item in value.reshape(-1).tolist()]
        if isinstance(value, (list, tuple)):
            flat: list[int] = []
            for item in value:
                flat.extend(self._flatten_token_ids(item))
            return flat
        if isinstance(value, (int, np.integer)):
            return [int(value)]
        return []

    def _format_token_debug(self, token_ids: list[int]) -> str:
        if not token_ids:
            return "count=0 ids=[]"
        clipped = token_ids[: self.debug_realtime_token_limit]
        suffix = "" if len(token_ids) <= len(clipped) else f"...(+{len(token_ids) - len(clipped)} more)"
        message = f"count={len(token_ids)} ids={clipped}{suffix}"
        if not self.debug_realtime_decode:
            return message
        tokenizer = self.debug_tokenizer
        if tokenizer is None:
            return message
        try:
            decoded = tokenizer.decode(clipped, skip_special_tokens=False)
            return f'{message} decoded={decoded!r}'
        except Exception:
            return message

    def _log_realtime_tokens(self, *, direction: str, stage: str, token_ids: Any, extra: str = "") -> None:
        if not self._is_realtime_debug_enabled():
            return
        flat = self._flatten_token_ids(token_ids)
        if not flat:
            logger.info("realtime_tokens %s stage=%s count=0 %s", direction, stage, extra)
            return
        payload = self._format_token_debug(flat)
        if extra:
            logger.info("realtime_tokens %s stage=%s %s %s", direction, stage, payload, extra)
        else:
            logger.info("realtime_tokens %s stage=%s %s", direction, stage, payload)

    def _extract_sampler_token_ids(self, sampler_output: Any) -> list[int]:
        ...
        for attr in (
            ...
            if hasattr(sampler_output, attr):
                flat = self._flatten_token_ids(getattr(sampler_output, attr))
                if flat:
                    return flat
        if isinstance(sampler_output, dict):
            for key in ("sampled_token_ids", "token_ids", "output_token_ids", "next_token_ids", "tokens"):
                flat = self._flatten_token_ids(sampler_output.get(key))
                if flat:
                    return flat
        return []
    ...
    def forward(
        ...
        **kwargs: object,
        ...
        """
        self._log_realtime_tokens(
            direction="input",
            stage=self.model_stage,
            token_ids=input_ids,
            extra=f"logits_index={logits_index}" if logits_index is not None else "",
        )
    ...
    def sample(
        ...
        self._log_realtime_tokens(
            direction="output",
            stage=self.model_stage,
            token_ids=self._extract_sampler_token_ids(output),
        )
        return output
